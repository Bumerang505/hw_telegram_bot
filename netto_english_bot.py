...
def add_new_word(message):
    new_ru_word = bot.get_updates()[-1].message.text
    ...
    check_word = session.query(Words.ru_word).filter(Words.ru_word.ilike(new_ru_word)).all()
    if len(check_word) == 0:
...
def true_translate(ru_word):
    select = session.query(Words.translate).filter(Words.ru_word == ru_word).all()
...
def delete_word_button(message):
    ...
    def delete_word(msg):
        word_for_delete = bot.get_updates()[-1].message.text
        word_id_in_vocabulary = (session.query(Vocabulary).
                                 ...
                                 filter(Users.uid == msg.chat.id, Words.ru_word.ilike(word_for_delete))).first()

        if word_id_in_vocabulary:
            session.delete(word_id_in_vocabulary)
            session.commit()
            print(f'Слово {word_for_delete} удалено из словаря')

        else:
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            markup.add(Buttons.NEXT, Buttons.ADD_WORD, Buttons.DELETE_WORD)
            bot.send_message(msg.chat.id, f'Слово {word_for_delete} не найдено в словаре, '
                                          f'проверьте правильность его написания.',
                             reply_markup=markup)
